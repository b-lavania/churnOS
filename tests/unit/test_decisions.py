...
def test_emit_records_ranked_by_cost():
    ws = build_workspace(get_preset("assistant_heavy"), seed=42, n_sessions=_N)
...
def test_override_without_reclassify():
    ws = build_workspace(get_preset("ops_mission"), seed=7, n_sessions=_N)
...
def test_write_outcome_fields():
    ws = build_workspace(get_preset("workspace_crm"), seed=3, n_sessions=_N)
